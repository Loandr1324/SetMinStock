...
def payment(df_payment):
    ...
    if USE_OPT_STORE:
        df_payment['Компания MaCar продажи'] = df_payment.filter(regex=columns_sales[:-14]).sum(axis=1)
    else:
        df_payment['Компания MaCar продажи'] = df_payment.filter(regex=columns_sales).sum(axis=1)
